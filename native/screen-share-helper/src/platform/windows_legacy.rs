...
use windows::{
    ...
    Win32::{
        Foundation::{CloseHandle, FILETIME, HMODULE, HWND, LPARAM, RECT},
        Graphics::{
        ...
        System::{
            ...
            Threading::{
                GetCurrentProcessId, GetProcessTimes, OpenProcess, PROCESS_NAME_WIN32,
...
pub struct PreparedCapture {
    ...
    audio_isolation_watch: Option<AudioIsolationWatch>,
...
enum AudioIsolationWatchKind {
    Display { proof: WebViewProcessProof },
    Application { process_id: u32, creation_time: u64 },
}

struct AudioIsolationWatch {
    kind: AudioIsolationWatchKind,
                ...
                ProcessLoopbackTarget::IncludeProcessTree(process_id) => {
                    let Some(creation_time) = process_creation_time(process_id) else {
                        return (
                            None,
                            Some(
                                "Windows could not fingerprint the selected application process; video is still available."
                                    .to_string(),
                            ),
                            None,
                        );
                    };
                    AudioIsolationWatchKind::Application {
                        process_id,
                        creation_time,
                    }
                }
            };
            let watch = Some(AudioIsolationWatch {
                kind,
...
async fn start_process_audio_capture(
    ...
    mut isolation_watch: Option<AudioIsolationWatch>,
) -> Result<ProcessAudioCapture, String> {
    if let Some(watch) = isolation_watch.as_ref()
        && !audio_isolation_is_current(watch, &watch.receiver.borrow())
    {
        return Err(format!(
            "[audio-isolation-unavailable] {DISPLAY_AUDIO_ISOLATION_REASON}"
        ));
        ...
        tokio::spawn(async move {
            loop {
                ...
                if should_stop_for_isolation_change(
                    ...
                    let _ = isolation
                        .failure_sender
                        .send(AUDIO_ISOLATION_CHANGED.to_string());
                    break;
                }
            }
        })
...
fn audio_isolation_is_current(watch: &AudioIsolationWatch, state: &WebViewProcessState) -> bool {
    let Some(current_proof) = state.proof() else {
        return false;
    };
    let Ok(parents) = process_parent_map() else {
        return false;
    };
    if !current_proof.is_valid_for(&parents) {
        return false;
    }
    match &watch.kind {
        AudioIsolationWatchKind::Display { proof } => {
            display_audio_isolation_is_current(proof, state)
        }
        AudioIsolationWatchKind::Application {
            process_id,
            creation_time,
        } => {
            process_creation_time(*process_id) == Some(*creation_time)
                && application_process_loopback_target(
                    *process_id,
                    unsafe { GetCurrentProcessId() },
                    Some(current_proof),
...
fn should_stop_for_isolation_change(
    stop: &AtomicBool,
    watch: &AudioIsolationWatch,
    state: &WebViewProcessState,
) -> bool {
    !audio_isolation_is_current(watch, state) && !stop.swap(true, Ordering::AcqRel)
}

#[cfg(test)]
fn should_stop_for_display_isolation_change(
    stop: &AtomicBool,
    expected: &WebViewProcessProof,
    state: &WebViewProcessState,
) -> bool {
...
mod tests {
    ...
    fn an_electron_topology_change_invalidates_active_audio() {
        ...
        assert!(should_stop_for_display_isolation_change(
            &stop,
            &initial,
            &WebViewProcessState::Proven(changed.clone())
        ));
